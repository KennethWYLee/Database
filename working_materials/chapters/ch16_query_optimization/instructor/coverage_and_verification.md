...
- Official slide deck `from_11001_DB/PowerPoint Presentations/ch16.pdf`, slides
  16.1-16.86.
...
Required instruction covers expression equivalence; selection cascade and pushdown;
projection pushdown with retained keys; inner-join reorder and Cartesian-product risk;
outer-join counterexamples; catalog tuple/distinct/index statistics; uniform selectivity
assumptions and skew; `ANALYZE`; and practical `EXPLAIN QUERY PLAN` interpretation.
...
| C16.03 | selection pushdown | 16.2.1-16.2.2, pp. 748-754; slides 16.7-16.15 | student/course filters | reject unconditional early-filter rule |
| C16.04 | projection pushdown | 16.2.1-16.2.2, pp. 749, 754 | retain join/filter columns | find failure after removing key |
| C16.05 | join order/product risk | 16.2.3, pp. 754-755; slides 16.16-16.19 | 100 students to 500 enrollments | classify A-B-C orders |
| C16.06 | outer-join boundary | 16.2.1, pp. 750-752; slides 16.11-16.14 | Department 101 NULL row | execute WHERE/ON counterexample |
| C16.07 | catalog/statistics | 16.3.1, pp. 758-760; slides 16.36-16.42 | `ANALYZE` and `sqlite_stat1` | identify missing skew detail |
| C16.08 | selectivity/skew | 16.3.2, pp. 760-762; slides 16.43-16.47 | 99/1 event distribution | calculate actual selectivity |
...
After the Chapter 15 plan examples, students establish equivalence before discussing
performance, rewrite safe selections and projections, and compare join orders. They
then execute the outer-join counterexample, inspect `sqlite_stat1`, compare uniform
estimates with skewed data, and revise one unsupported performance claim.

## Verification command

```powershell
py -3 working_materials/chapters/ch16_query_optimization/instructor/verify_ch16.py
```
...
- Files created: student guide, SQL lab, instructor source/alignment record, and
  verifier.
- Source verification: complete for textbook printed pages 743-788 and slides
  16.1-16.86.
