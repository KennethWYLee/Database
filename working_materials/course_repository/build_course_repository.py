...
PREVIEW_DIR = OUTPUT_DIR / "database_course_repository"
...
def render_schedule(config: dict) -> str:
    ...
    for week in config["weeks"]:
        if week["materials"]:
            materials = ", ".join(
                f"[{chapter_id.upper()} {titles[chapter_id]}]({chapter_id}.ipynb)"
                for chapter_id in week["materials"]
            )
        else:
            materials = "None"
        lines.append(
...
def build_preview(config: dict) -> None:
    ...
    for generated in (PREVIEW_DIR, MANIFEST_PATH):
    ...
    for chapter in config["chapters"]:
        ...
        target = safe_target(f"{chapter['id']}.ipynb")
        target.write_text(
...
def write_manifest(config: dict) -> list[dict]:
    ...
    manifest = {
        ...
        "content_status": "Unified local course repository preview; not published",
        "files": files,
    }
...
def verify_content(config: dict) -> None:
    ...
    if actual_files != expected_files:
        errors.append(
            f"Expected only the unified root files; missing={sorted(expected_files - actual_files)}, unexpected={sorted(actual_files - expected_files)}"
        )
        ...
        if path.suffix == ".ipynb":
            ...
            if path.suffix == ".md":
                for match in MARKDOWN_LINK_PATTERN.finditer(content):
                    target_text = match.group(1).strip().split("#", 1)[0]
