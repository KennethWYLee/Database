...
- 已完整閱讀工作區及課程治理文件、`PROJECT.md`、`README.md`、`COURSE_PLAN.md`、
  `Intro DB/syllabus.md`、章節教材設計prompt、working
...
- `maintenance/student_sqlite_package/package_files.json`是學生套件唯一
...
- 逐一執行`python maintenance/chapters/<chapter>/instructor/verify_chXX.py`，
...
- 執行`python maintenance/student_sqlite_package/build_package.py --verify`，
...
- `README.md`、`PROJECT.md`、`COURSE_PLAN.md`、`maintenance/README.md`及
...
3. `COURSE_PLAN.md`第六至第八節：確認每週範圍、三次考試coverage及Class
   Performance規則。
4. `Intro DB/syllabus.md`的Weekly Schedule、Assessment
   and Grading及Schedule Notes：確認可對學生公布的英文表述。
5. `PROJECT.md`的Repository與發布狀態：建立第一批GitHub公開allow-list及獨立公開位置。

## 10. 主要下一步

2026-09-07更新：本機與GitHub改用相同的main目錄，`Intro DB/`集中課綱與12份
章節notebook。維護來源搬到`maintenance/`，Week 1練習允許修正或解釋正確預測。
本次移動與驗證詳見`structure_migration_review.md`；尚未commit/push。主要後續
動作是另獲授權後同步同一main，完成條件為來源、成品、遠端路徑與連結一致，
不再同步不同結構的預覽分支。下段為2026-09-05的歷史紀錄。
